...
CData's Model Context Protocol (MCP) Server for Neo4J

:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Neo4J (beta)](https://www.cdata.com/download/download.aspx?sku=LNZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Neo4J supported by the [CData JDBC Driver for Neo4J](https://www.cdata.com/drivers/neo4j/jdbc).

CData JDBC Driver connects to Neo4J by exposing them as relational SQL models.

This server wraps that driver and makes Neo4J data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.

## Setup Guide
1. Clone the repository:
      ```bash
      git clone https://github.com/cdatasoftware/neo4j-mcp-server-by-cdata.git
      cd neo4j-mcp-server-by-cdata
      ```
2. Build the server:
      ```bash
      mvn clean install
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/neo4j/download/jdbc](https://www.cdata.com/drivers/neo4j/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Neo4J\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Neo4J/`
    * Run the command `java -jar cdata.jdbc.neo4j.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.neo4j.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `neo4j.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.neo4j.Neo4jDriver)
